...
public class EncodingFilter implements Filter { 
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		// resp 응답정보로부터 출력을 위한 객체 얻어오기
		PrintWriter out = resp.getWriter();
		out.println("<h6>Encoding Filter 작동 전</h6>");
		
		// 실제 필터가 처리하는 내용
		logger.info("[EncodingFilter] doFilter"); //로거 찍기
		req.setCharacterEncoding(encoding);  //요청 정보의 인코딩 설정 
		resp.setContentType("text/html"); 	 //응답 정보의 타입 설정
		resp.setCharacterEncoding(encoding); //응답 정보의 인코딩 설정
		
		// chain
		//요청 처리 종료 후 다음 필터로 전달해준다 
	...
	@Override
	public void destroy() {
		//Filter.super.destroy(); //필요없음 주석처리해주기
		
		logger.info("[EncodingFilter] destroy"); //로거 찍기
	}
